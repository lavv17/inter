...
void  ReadInEvents(void)
{
   ...
   while(peek(0,0x41A)!=peek(0,0x41C))
   {
      ...
      newevent.time=Timer();
      enable();
      if((newevent.type&0xFF00)==0xE000)
         newevent.type=(newevent.type<<8)&0xFFFF;
   ...
   new_mouse_x=(r.x.cx+4)/8;
   new_mouse_y=(r.x.dx+4)/8;
   new_buttons=((r.x.bx&1)?LEFT_BUTTON:0)
               |((r.x.bx&2)?RIGHT_BUTTON:0)
               |((r.x.bx&4)?CENTER_BUTTON:0);
...
void  DosInit(void)
{
   ...
   if(r.x.ax==0xFFFF)
   {
      ...
      r.x.ax=3;  /* query mouse pos and buttons */
      int86(0x33,&r,&r);
      old_mouse_x=I_MouseX=(r.x.cx+4)/8;
      old_mouse_y=I_MouseY=(r.x.dx+4)/8;
      old_buttons=I_Buttons=((r.x.bx&1)?LEFT_BUTTON:0)
                           |((r.x.bx&2)?RIGHT_BUTTON:0)
                           |((r.x.bx&4)?CENTER_BUTTON:0);
   }
